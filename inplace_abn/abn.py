...
class GhostABN(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, activation="leaky_relu",
        ...
        if self.affine:
            self.weight = nn.Parameter(torch.ones(num_features*self.num_splits))
            self.bias = nn.Parameter(torch.zeros(num_features*self.num_splits))
        else:
            self.register_parameter('weight', None)
            self.register_parameter('bias', None)
    ...
    def forward(self, x):
        ...
        x = functional.batch_norm(
                x.view(-1, C*self.num_splits, H, W), self.running_mean, self.running_var, 
...
#                                   self.training, self.momentum, self.eps)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
        ...
        super(GhostABN, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys,
                                               error_msgs, unexpected_keys)
...
class InPlaceGhostABN(GhostABN):
    """InPlace Activated Ghost Batch Normalization
    ...
    def forward(self, x):
        N, C, H, W = x.shape
        return inplace_abn(x.view(-1, C*self.num_splits, H, W), self.weight.repeat(self.num_splits),
                           self.bias.repeat(self.num_splits), self.running_mean, self.running_var,
                           self.training, self.momentum, self.eps, self.activation, self.activation_param).view(N, C, H, W) 
